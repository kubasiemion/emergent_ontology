"""
train.py — cap5_cap4trained_acc80
==========================================
Cap5 architecture (n_counts=5, n_tokens=7) trained only on cap4 sequences.
Curriculum: cap1 → cap2 → cap3 → cap4. Stage 5 is never run.
C4 receives zero gradient throughout — it is a ghost state.

Saves weights.pt when cap4 accuracy first reaches 80% for 2 consecutive epochs.

Usage (from repo root):
    python demos/wynns_world/models/pretrained/cap5_cap4trained_acc80/train.py
"""
...
N_COUNTS        = 5    # architecture: 5 count states, 7 tokens
TRAIN_STAGES    = 4    # curriculum stops at cap4 — C4 never trained
EMB_DIM         = 8
HIDDEN_DIM      = 16
N_SEQS          = 2000
SEQ_LEN         = 200
