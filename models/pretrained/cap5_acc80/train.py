...
train.py — cap5_acc80
=====================================
Trains a BoxedCounter to track a linear (non-wrapping) count in [0, 4].
Uses curriculum training: cap1 → cap2 → cap3 → cap4 → cap5.
Saves weights.pt when cap5 accuracy first reaches 80% for 2 consecutive epochs.

Usage (from repo root):
    python demos/wynns_world/models/pretrained/cap5_acc80/train.py
"""
...
def main():
    ...
    if hit:
        ...
        print(f"\nMax epochs reached without hitting {TARGET_ACC:.0%} on cap{N_COUNTS}.")
