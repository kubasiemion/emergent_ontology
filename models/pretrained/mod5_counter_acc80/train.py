"""
train.py — mod5_counter_acc80
==============================
Trains a BoxedCounter to track a modular (wrapping) count in [0, 4].
INC at 4 wraps to 0; DEC at 0 wraps to 4.
Saves weights.pt when read accuracy first reaches 80% for 2 consecutive epochs.

Usage (from repo root):
    python demos/wynns_world/models/pretrained/mod5_counter_acc80/train.py
"""
...
def main():
    ...
    inputs, targets, masks = make_dataset(N_COUNTS, N_SEQS, SEQ_LEN, P_READ, SEED, modular=MODULAR)
    ...
    for ep in range(1, MAX_EPOCHS + 1):
        ...
        print(f"ep {ep:03d}  nll {total_nll/max(1,total_batches):.4f}  acc {acc:.4f}", end="")
        ...
        print(f"\nMax epochs reached without hitting {TARGET_ACC:.0%}.")
